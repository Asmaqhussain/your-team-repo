# S8 Evidence Index

## Architecture
- [Context diagram](../docs/architecture/context.png)
- [Component diagram](../docs/architecture/components.png)
- [Interfaces](../docs/architecture/openapi.yaml)
- [NFR mapping](../docs/architecture/nfr_mapping.csv)
- [Trade-offs note](../docs/architecture/tradeoffs.md)

## Verification
- [Test script](./tests/T-PERF-01.js)
- [Evidence artifact](../evidence/EV-perf-rsvp.html)

## Review notes
- [Team notes](./review_notes/TEAM_X.md)

## Access
Instructors have read access; branch protections and CI are in place (see S7).

